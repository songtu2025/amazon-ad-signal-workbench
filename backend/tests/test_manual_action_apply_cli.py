...
def test_manual_action_apply_rewrites_after_voided_legacy_todo_with_isolated_roots(tmp_path: Path) -> None:
    ...
    assert len(written["written_record"]["evidence_snapshot"]) == 25
    ...
    assert smoke["written_evidence_snapshot_count"] == 25
    assert smoke["post_write_max_manual_action_evidence_snapshot_count"] == 25
    assert smoke["post_write_review_todo_evidence_snapshot_counts"] == {"7d": 25, "14d": 25}
