...
def test_beach_essentials_manual_action_api_roundtrip_reads_back_evidence_snapshot(monkeypatch, tmp_path: Path) -> None:
    ...
    assert preflight["evidence_snapshot_preview"]["item_count"] == 29
    ...
    assert snapshot_by_label["语义组"]["source"] == "规则语义"
    ...
    assert len(action["evidence_snapshot"]) == 29
    ...
    assert {todo["evidence_snapshot"][3]["label"] for todo in todos} == {"语义组"}
    ...
    assert {todo["evidence_snapshot"][7]["label"] for todo in todos} == {"同组投放商品表现"}
    assert {todo["evidence_snapshot"][8]["label"] for todo in todos} == {"逐投放上下文"}
    assert {todo["evidence_snapshot"][9]["label"] for todo in todos} == {"投放词证据"}
    assert {todo["evidence_snapshot"][10]["label"] for todo in todos} == {"搜索词边界"}
    assert {todo["evidence_snapshot"][11]["label"] for todo in todos} == {"广告位边界"}
    assert {todo["evidence_snapshot"][12]["label"] for todo in todos} == {"广告位活动级背景"}
    assert {todo["evidence_snapshot"][13]["label"] for todo in todos} == {"ABA 背景"}
    assert {
        "自动归因到单个广告 ASIN" in todo["evidence_snapshot"][6]["detail"] for todo in todos
    } == {True}
