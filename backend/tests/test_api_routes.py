...
def test_beach_essentials_manual_action_api_roundtrip_reads_back_evidence_snapshot(monkeypatch, tmp_path: Path) -> None:
    ...
    assert preflight["evidence_snapshot_preview"]["item_count"] == 24
    ...
    assert len(action["evidence_snapshot"]) == 24
    ...
    assert [len(todo["evidence_snapshot"]) for todo in todos] == [24, 24]
    ...
    assert {todo["evidence_snapshot"][8]["label"] for todo in todos} == {"ABA 背景"}
    assert {
