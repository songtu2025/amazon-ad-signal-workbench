...
from app.services.parent_asin_import import (
    DEFAULT_PARENT_ASIN_SNAPSHOT_ROOT,
    PARENT_ASIN_MAP_FILE,
    SOURCE_TABLE,
    SOURCE_TYPE,
)


def load_parent_asin_rows_from_latest_snapshot(snapshot_root: Path = DEFAULT_PARENT_ASIN_SNAPSHOT_ROOT) -> list[dict[str, Any]]:
    snapshot_dir, manifest = _latest_parent_asin_snapshot(Path(snapshot_root))
    if snapshot_dir is None or manifest is None:
        return []

    map_path = snapshot_dir / "normalized" / PARENT_ASIN_MAP_FILE
...
def _latest_parent_asin_snapshot(snapshot_root: Path) -> tuple[Path | None, dict[str, Any] | None]:
