    ...
    *,
    ...
    if not normalized_scope_id:
        return search_intent_summaries(
            ...
            data_grain="当前站点广告中实际产生表现的 ad_search_term_daily_metrics 用户搜索词表现行，按标准化用户搜索词及规则归类聚合",
        )
...
def _search_intent_data_grain_for_product_scope(product_scope_id: str) -> str:
    if product_scope_id.startswith("parent_asin:"):
        return "当前 Parent ASIN 关联广告上下文中的 ad_search_term_daily_metrics 用户搜索词表现行，按标准化用户搜索词及规则归类聚合；不包含未投放子 ASIN、自然搜索词或 ABA 站点数据"
    if product_scope_id.startswith("ad_asin:"):
        return "当前广告 ASIN 关联广告上下文中的 ad_search_term_daily_metrics 用户搜索词表现行，按标准化用户搜索词及规则归类聚合"
    if product_scope_id.startswith("sales_asin:"):
        return "当前经营 ASIN 关联广告上下文中的 ad_search_term_daily_metrics 用户搜索词表现行，按标准化用户搜索词及规则归类聚合"
    return "当前诊断入口关联广告上下文中的 ad_search_term_daily_metrics 用户搜索词表现行，按标准化用户搜索词及规则归类聚合"
