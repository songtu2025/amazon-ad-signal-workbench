...
def aba_phrase_context_evidence_items(aba_row: dict, row: dict) -> list[EvidenceItem]:
    ...
    return [
        EvidenceItem(
            ...
            source_type="积加API",
            source_name="搜索词规则语义",
            metric_name="search_intent_group",
    ...
    for (market_id, query), group in groups.items():
        ...
        suggested_action = (
            ...
            if is_asin_like_group
                ...
                update={
                    "description": (
                        f"先按 Parent ASIN 广告搜索词表现复核「{action_semantic_group}」核对同类搜索词表现，"
...
def detect_search_intent_grouping_data_quality(rows: list[dict]) -> list[AiSignal]:
    ...
    return [
        AiSignal(
            ...
            risk="如果忽略该问题，运营可能把未分组搜索词的整体表现误读成同一 Parent ASIN 广告搜索词表现复核分组。",
...
def search_intent_proves(data_grain: str) -> str:
    ...
    return f"能证明{scope}内按标准化用户搜索词及规则归类聚合后的同类广告搜索词花费、点击、订单和 ABA 背景。"
...
def search_intent_summaries(
    ...
    *,
    ...
    data_grain: str = "当前诊断入口关联广告上下文中的 ad_search_term_daily_metrics 用户搜索词表现行，按标准化用户搜索词及规则归类聚合",
