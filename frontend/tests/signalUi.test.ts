...
assertEqual(searchTermOpportunityReviewChain?.reviewLayers.length, 3);
...
assertEqual(searchTermOpportunityReviewChain?.reviewLayers[2].label, "广告位边界");
assertIncludes(searchTermOpportunityReviewChain?.reviewLayers[2].purpose ?? "", "搜索词直连、广告组级、广告活动级");
assertIncludes(searchTermOpportunityReviewChain?.reviewLayers[2].doesNotProve ?? "", "不能证明广告位导致");
