...
assertEqual(productScopeDiagnosisBrief.decisionGuide.quickTriage.length, 3);
assertEqual(productScopeDiagnosisBrief.decisionGuide.quickTriage[0].label, "先看");
assertIncludes(productScopeDiagnosisBrief.decisionGuide.quickTriage[0].value, "RBK004");
assertIncludes(productScopeDiagnosisBrief.decisionGuide.quickTriage[0].detail, "投放结构失衡");
assertEqual(productScopeDiagnosisBrief.decisionGuide.quickTriage[1].label, "为什么");
assertIncludes(productScopeDiagnosisBrief.decisionGuide.quickTriage[1].detail, "证据");
assertEqual(productScopeDiagnosisBrief.decisionGuide.quickTriage[2].label, "现在做");
assertIncludes(productScopeDiagnosisBrief.decisionGuide.quickTriage[2].value, "人工复核");
...
assertIncludes(recommendedAdGroupEvidenceBrief.decisionGuide.quickTriage[0].value, "推荐搜索词 beach essentials");
assertIncludes(recommendedAdGroupEvidenceBrief.decisionGuide.quickTriage[0].detail, "2 个广告组只作为证据入口");
assertIncludes(recommendedAdGroupEvidenceBrief.decisionGuide.quickTriage[1].value, "搜索词证据可回到广告组");
...
assertEqual(selectedSearchIntentFocusContext.pathItems[1]?.label, "二级搜索词证据聚焦");
...
assertIncludes(selectedSearchIntentFocusContext.relation, "临时聚焦同类 SearchTerm 信号");
assertIncludes(selectedSearchIntentFocusContext.relation, "沿广告组、投放词、同组广告 ASIN、广告位逐层核对证据");
...
assertEqual(selectedSearchIntentTermReasonSummary.executionSteps.length, 6);
...
assertEqual(selectedSearchIntentTermReasonSummary.executionSteps[2]?.label, "3. 核对广告组 / 投放词");
assertIncludes(selectedSearchIntentTermReasonSummary.executionSteps[2]?.value ?? "", "广告活动 / 广告组 / 投放词");
assertIncludes(selectedSearchIntentTermReasonSummary.executionSteps[2]?.detail ?? "", "广告组是容器");
assertEqual(selectedSearchIntentTermReasonSummary.executionSteps[3]?.label, "4. 核对同组广告 ASIN");
assertIncludes(selectedSearchIntentTermReasonSummary.executionSteps[3]?.value ?? "", "advertised products");
assertIncludes(selectedSearchIntentTermReasonSummary.executionSteps[3]?.detail ?? "", "不能直接归因到单个 ASIN");
assertEqual(selectedSearchIntentTermReasonSummary.executionSteps[4]?.label, "5. 核对广告位");
assertIncludes(selectedSearchIntentTermReasonSummary.executionSteps[4]?.value ?? "", "Top of Search");
assertIncludes(selectedSearchIntentTermReasonSummary.executionSteps[4]?.detail ?? "", "只能说明流量分布");
assertEqual(selectedSearchIntentTermReasonSummary.executionSteps[5]?.label, "6. 选择人工动作");
assertIncludes(selectedSearchIntentTermReasonSummary.executionSteps[5]?.value ?? "", "打开具体 SearchTerm 后人工复核");
assertIncludes(selectedSearchIntentTermReasonSummary.executionSteps[5]?.detail ?? "", "7/14 天后再读指标复盘");
