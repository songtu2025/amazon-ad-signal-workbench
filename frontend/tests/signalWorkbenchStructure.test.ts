...
assertIncludes(workbenchSource, 'aria-label="SearchTerm 机会筛选与当前信号关系"');
...
assertIncludes(workbenchSource, "SearchTerm 机会筛选");
assertIncludes(workbenchSource, "当前诊断入口内的广告搜索词机会语义聚合");
assertIncludes(workbenchSource, "诊断入口保持不变，仅显示同语义组 SearchTerm 机会；不做商品归因");
assertIncludes(workbenchSource, "card.purpose");
assertIncludes(workbenchSource, "card.boundary");
assertIncludes(workbenchSource, "SearchTerm 机会筛选不切换经营商品或广告组");
assertIncludes(workbenchSource, "当前 SearchTerm 机会筛选暂无对应机会");
assertIncludes(workbenchSource, 'aria-label="SearchTerm 机会筛选（语义聚合）"');
...
const searchIntentReviewPanelIndex = workbenchSource.indexOf('aria-label="SearchTerm 机会筛选（语义聚合）"');
...
assert(searchIntentReviewPanelIndex > queueTabsIndex, "SearchTerm 机会筛选必须跟在队列筛选之后");
assert(searchIntentReviewPanelIndex < signalRowsIndex, "SearchTerm 机会筛选必须贴近信号队列，而不是作为独立主入口");
