...
assertIncludes(workbenchSource, "buildProductScopeDiagnosisBrief(productScopeFirstScreenSummary, productScopeEvidenceRouteGuide, productScopeAdGroupDiagnosisForCurrentFocus)");
...
assertIncludes(workbenchSource, "动作对象：");
assertIncludes(workbenchSource, "来源读回：");
assertIncludes(workbenchSource, "允许动作：记录观察 / 标记已处理 / 加入复盘 / 忽略本次");
assertIncludes(workbenchSource, "右侧只核对能否留痕或加入复盘，不重复解释 Parent ASIN 排序理由，也不执行广告操作。");
...
assert(reviewTodoPathReadbackPreviewIndex > reviewFlowDecisionSummaryIndex, "复盘默认摘要必须读回复盘待办诊断路径");
