...
assertIncludes(workbenchSource, 'aria-label="单屏搜索词默认路径"');
assertIncludes(workbenchSource, 'aria-label="单屏搜索词完整复核顺序"');
assertIncludes(workbenchSource, "searchIntentSummary.priorityPathItems[0]");
assertIncludes(workbenchSource, "searchIntentSummary.priorityPathItems.slice(1).map");
assertIncludes(workbenchSource, 'aria-label="搜索词复核默认路径"');
assertIncludes(workbenchSource, 'aria-label="搜索词复核完整顺序"');
...
assertIncludes(workbenchSource, "rows.find((row) => row.id === selectedId) ?? rows[0]");
assertIncludes(workbenchSource, "默认只看当前聚焦");
assertIncludes(workbenchSource, 'aria-label="其他广告组完整候选"');
assertIncludes(workbenchSource, "展开其他 {otherRows.length} 个广告组候选");
...
assertIncludes(workbenchSource, 'aria-label="Parent ASIN 快速决策摘要"');
assertIncludes(workbenchSource, "brief.decisionGuide.quickTriage.map");
...
assertIncludes(workbenchSource, "otherRows.map((row) => (");
assertIncludes(workbenchSource, "aria-pressed={false}");
...
assertIncludes(signalUiSource, "ProductScopeDecisionGuideQuickItem");
assertIncludes(signalUiSource, "quickTriage");
...
assertIncludes(signalUiSource, "整体销售盘和广告承接占比");
assertIncludes(signalUiSource, "经营盘多大、广告承接占多少");
assertIncludes(signalUiSource, "不把全部销售子 ASIN 当广告对象");
assertIncludes(signalUiSource, "未投放变体不进入广告分析");
...
assertIncludes(signalUiSource, "优先原因");
assertIncludes(signalUiSource, "证据合流");
assertIncludes(signalUiSource, "按问题落点、证据合流和人工可执行性");
assertIncludes(signalUiSource, "不是扫完整广告组列表");
...
assertIncludes(signalUiSource, "先给默认核对层");
assertIncludes(signalUiSource, "默认先查");
assertIncludes(signalUiSource, "证据路径仍保留");
assertIncludes(signalUiSource, "避免用户从四类明细里重新找入口");
...
assertIncludes(signalUiSource, "3. 核对广告组 / 投放词");
assertIncludes(signalUiSource, "4. 核对同组广告 ASIN");
assertIncludes(signalUiSource, "5. 核对广告位");
assertIncludes(signalUiSource, "6. 选择人工动作");
assertIncludes(signalUiSource, "广告组是容器，不是商品");
assertIncludes(signalUiSource, "只能作为候选承接，不能直接归因到单个 ASIN");
assertIncludes(signalUiSource, "广告位只能说明流量分布");
...
assertIncludes(workbenchSource, "pendingEvidenceDrilldownScopeId");
assertIncludes(workbenchSource, "handleOpenProductScopeEvidenceDrilldown");
assertIncludes(workbenchSource, "handleOpenTopProductScopeEvidenceDrilldown");
assertIncludes(workbenchSource, "keepPendingEvidenceDrilldown: true");
assertIncludes(workbenchSource, "signalTriageSummary?.product_scope_gate?.selected_product_scope_id?.trim()");
...
assertIncludes(signalUiSource, "经营入口未切换");
...
assertIncludes(workbenchSource, 'aria-label="搜索词复核默认路径"');
assertIncludes(workbenchSource, 'aria-label="搜索词复核完整顺序"');
assertIncludes(workbenchSource, "const topPriorityPathItem = summary.priorityPathItems[0] ?? null;");
assertIncludes(workbenchSource, "const remainingPriorityPathItems = summary.priorityPathItems.slice(1);");
assertIncludes(workbenchSource, "remainingPriorityPathItems.map");
...
assertIncludes(workbenchSource, "经营诊断入口未切换；这里只临时聚焦同类 SearchTerm 信号，帮助打开具体搜索词证据链；不做商品归因");
assertIncludes(workbenchSource, 'aria-label="当前广告搜索词证据聚焦"');
assertIncludes(workbenchSource, "退出聚焦");
...
assertIncludes(workbenchSource, "当前 Parent ASIN 广告搜索词证据聚焦：{activeSearchIntentLabel}");
...
assert(searchIntentReviewStatusIndex < searchIntentReviewCardSnapshotIndex, "闭环状态必须先于默认判断依据，避免用户先读裸指标");
...
assertIncludes(workbenchSource, "const defaultDrilldown = productScopePriorityDefaultDrilldown(topItem)");
assertIncludes(workbenchSource, "先看谁");
assertIncludes(workbenchSource, "打开后看哪层");
assertIncludes(workbenchSource, "value: defaultDrilldown.value");
...
assertIncludes(workbenchSource, "打开并定位广告证据");
assertIncludes(workbenchSource, "并定位广告证据下钻");
assertIncludes(workbenchSource, "handleSelectProductScopePriority");
assertIncludes(workbenchSource, "onOpenTop={handleOpenTopProductScopeEvidenceDrilldown}");
...
assertIncludes(workbenchSource, 'aria-label="Parent ASIN 经营销售盘与广告证据概览"');
...
assertIncludes(stylesSource, ".productScopeQuickTriage");
...
assertIncludes(workbenchSource, "const priorityRow = rows.find((row) => row.id === selectedId) ?? rows[0];");
assertIncludes(workbenchSource, "const otherRows = priorityRow ? rows.filter((row) => row.id !== priorityRow.id) : rows;");
...
assertIncludes(workbenchSource, "1. 当时判断");
assertIncludes(workbenchSource, "2. 当时能证明");
assertIncludes(workbenchSource, "3. 当时不能证明");
...
assertIncludes(workbenchSource, 'aria-label="复盘回看四问"');
assertIncludes(workbenchSource, "复盘回看四问");
assertIncludes(workbenchSource, "沿点击时证据快照");
assertIncludes(workbenchSource, "1. 当时判断");
assertIncludes(workbenchSource, "2. 当时能证明");
assertIncludes(workbenchSource, "3. 当时不能证明");
assertIncludes(workbenchSource, "4. 现在复盘做什么");
...
assertIncludes(workbenchSource, "selectedManualActionPostWriteReviewMetric");
...
assertIncludes(workbenchSource, "复盘看什么");
assertIncludes(workbenchSource, "7/14 天到期后对比花费、订单、销售额、ACOS、CVR 和广告位 / 搜索词边界");
...
assertIncludes(workbenchSource, "\"复盘指标\"");
...
assertIncludes(workbenchSource, "处理前后指标已可人工复核");
assertIncludes(workbenchSource, "等待指标可复核");
assertIncludes(workbenchSource, "处理前后指标可复核和保存前检查");
